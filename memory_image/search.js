$.getJSON("/memory_image/search.json", function(json) {
...
function search(go) {
  ...
  if(results && results.length == 1) {
    window.location.replace("/memory_image/" + results[0].ref + "?highlight=" + escape(term));
...
function displayResults(results, term) {
  ...
  $.each(results, function(i, result) {
    ...
    container.append("<p><a href=\"/memory_image/" + result.ref + "?highlight=" + escape(term) + "\">" + article.title + " - " + article.subtitle + "</a></p>");
  });
}
