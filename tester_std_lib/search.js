...
$.getJSON("/tester_std_lib/" + "/" + search_json, function(json) {
...
function search(go) {
  ...
  if (window.index) {
    ...
    if(results && results.length === 1) {
      window.location = "/tester_std_lib/" + results[0].ref + "?highlight=" + escape(term);
...
function displayResults(results, term) {
  ...
  $.each(results, function(i, result) {
    ...
    container.append("<p><a href=\"/tester_std_lib/" + result.ref + "?highlight=" + escape(term) + "\">" + text + "</a></p>");
  });
}
