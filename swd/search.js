...
$.getJSON("/swd/" + "/" + search_json, function(json) {
...
function search(go) {
  ...
  if (window.index) {
    ...
    if(results && results.length === 1) {
      window.location = "/swd/" + results[0].ref + "?highlight=" + escape(term);
...
function displayResults(results, term) {
  ...
  $.each(results, function(i, result) {
    ...
    container.append("<p><a href=\"/swd/" + result.ref + "?highlight=" + escape(term) + "\">" + text + "</a></p>");
  });
}
