...
$.getJSON("/doc_helpers/" + "/" + search_json, function(json) {
...
function search(go) {
  ...
  if (window.index) {
    ...
    if(results && results.length === 1) {
      window.location = "/doc_helpers/" + results[0].ref + "?highlight=" + escape(term);
...
function displayResults(results, term) {
  ...
  $.each(results, function(i, result) {
    ...
    container.append("<p><a href=\"/doc_helpers/" + result.ref + "?highlight=" + escape(term) + "\">" + text + "</a></p>");
  });
}
