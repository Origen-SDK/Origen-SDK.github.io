$.getJSON("/debuggers/search.json", function(json) {
...
function search(go) {
  ...
  if(results && results.length == 1) {
    window.location.replace("/debuggers/" + results[0].ref + "?highlight=" + escape(term));
...
function displayResults(results, term) {
  ...
  $.each(results, function(i, result) {
    ...
    container.append("<p><a href=\"/debuggers/" + result.ref + "?highlight=" + escape(term) + "\">" + article.title + " - " + article.subtitle + "</a></p>");
  });
}
